...
echo [INFO] SSH target: !SERVER!
ssh %SSH_OPTS% !SERVER! bash -lc "cd !REMOTE_PATH! && git fetch origin !GIT_BRANCH! && (git stash push -u -m auto-deploy-pre-pull || true) && git checkout -B !GIT_BRANCH! origin/!GIT_BRANCH! && git reset --hard origin/!GIT_BRANCH!"
if errorlevel 1 (
...
ssh %SSH_OPTS% !SERVER! bash -lc "cd !REMOTE_PATH! && python3 -m venv venv && ./venv/bin/python -m pip install -r requirements.txt"
if errorlevel 1 (
